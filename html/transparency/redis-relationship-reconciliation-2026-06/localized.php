<?php
/**
 * Localized Transparency article renderer for Redis relationship reconciliation.
 */
...
$copy = [
  'de' => [
    'title' => 'Wie PayCal Geschäftsmitgliedschaften korrekt und auditierbar hält',
    'current' => 'Integrität von Geschäftsmitgliedschaften',
    'meta' => 'Wie PayCal Lebenszyklus-Datensätze von Geschäftsbeziehungen von Redis-Lookup-Indizes trennt, abgeleitete Indexabweichungen auditiert und aktive Mitgliedschaft, ausstehenden Zugriff und Eigentümerautorität sauber hält.',
    'deck' => 'Geschäftsmitgliedschaft ist mehr als eine Personenliste. Im Juni 2026 haben wir geschärft, wie PayCal Business Mitgliedschaften, Zugriffsanfragen, Einladungen und aktiven Zugriff in Redis darstellt. Dieser Artikel erklärt das Modell, die gefundene Abweichung, den Reparaturbefehl und warum die Änderung für Datenschutz, Dashboard-Korrektheit und künftige Auditierbarkeit wichtig ist.',
    ...
    'summary_rows' => [
      ['Betroffener Bereich', 'PayCal-Business-Beziehungslebenszyklen und Redis-Lookup-Indizes'],
      ...
      ['Kernkorrektur', 'Beziehungshashes bleiben kanonisch; aktive Mitglieder, ausstehende Beziehungen und nicht-terminale Lookup-Indizes sind getrennt'],
      ['Betriebstool', '<code>scripts/paycal business:relationships:audit</code> auditiert und repariert abgeleitete Indexabweichungen'],
    ...
    'problem_p' => [
      'PayCal Business nutzt Redis für schnelle Beziehungs-Lookups. Der kanonische Datensatz ist ein Hash wie <code>business:relationship:{businessId}:{userUUID}</code>. Dieser Hash speichert den Lebenszyklus einer Beziehung: Rolle, Status, Scopes, Zeitstempel, Einladungs- oder Anfragemetadaten und zugehörige Audit-Felder.',
      'Das Problem lag nicht im kanonischen Hash. Das Problem war semantische Abweichung in abgeleiteten Redis-Sets, die als Lookup-Indizes genutzt wurden. Historisch wurden <code>business:members:{businessId}</code> und <code>business:user:{userUUID}</code> manchmal als breite Beziehungsindizes verwendet. Dadurch konnte älterer Code eine ausstehende oder veraltete Beziehung zu leicht wie aktive Mitgliedschaft behandeln.',
      'Diese Unterscheidung ist wichtig. Eine ausstehende Zugriffsanfrage ist keine aktive Mitgliedschaft. Eine akzeptierte Beziehung ist nicht automatisch verschlüsselter Datenzugriff, solange erforderliche Zustimmung und Key-Wrap-Status nicht gültig sind. Ein abgeleitetes Redis-Set ist keine Autorität; es ist ein austauschbarer Index.',
    ],
    'model_intro' => 'Das gehärtete Modell trennt Lebenszyklusstatus von aktivem Zugriffsstatus:',
    'model_after' => 'Damit hat jedes Redis-Set genau eine Bedeutung. Dashboard-Zählungen können sich auf aktive Mitgliederindizes verlassen. Ausstehende Warteschlangen können <code>business:pending:{businessId}</code> verwenden. Beziehungsbewusste UI kann Lookup-Indizes nutzen, ohne aktiven Zugriff zu gewähren.',
    'drift_intro' => 'Ein veralteter Indexeintrag sieht so aus:',
    'drift_second' => 'Ein weiteres reparierbares Beispiel ist eine aktive Beziehung, der aktive Indizes fehlen:',
    'drift_after' => 'In beiden Fällen erfindet die Reparatur keine Beziehung. Sie gleicht nur abgeleitete Sets mit dem kanonischen Beziehungshash ab.',
    ...
    'repair_p' => ['Wir erstellten <code>scripts/business-relationships-audit.php</code> und machten es über den bestehenden internen Befehls-Dispatcher verfügbar:', 'Der Befehl hat zwei Modi:', 'Der Befehl wählt niemals automatisch Eigentümer aus, schreibt keine Rollenautorität um und erstellt keine kanonischen Beziehungen. Er gleicht nur austauschbare Indizes mit kanonischen Hashes ab.'],
    'modes' => ['<strong>Nur Bericht:</strong> scannt Beziehungshashes und meldet Abweichungen ohne Redis zu verändern.', '<strong>Reparatur:</strong> repariert bekannte abgeleitete Indexabweichungen durch Hinzufügen oder Entfernen von Redis-Set-Mitgliedern.'],
    ...
    'actions_l' => ['Aktive Mitgliedschaftsindizes von Beziehungslebenszyklus-Indizes getrennt.', '<code>business:relationships:{businessId}</code> und <code>business:relationships:user:{userUUID}</code> für nicht-terminalen Lookup hinzugefügt.', '<code>business:pending:{businessId}</code> für ausstehende Workflow-Sichtbarkeit hinzugefügt.', 'Den zentralen Beziehungsschreiber aktualisiert, sodass alle abgeleiteten Sets aus einem Pfad gepflegt werden.', 'Terminale Übergänge gehärtet, sodass widerrufene, abgelehnte, abgelaufene und zurückgezogene Beziehungen nicht direkt aktiv werden können.', 'Scope-Preset- und Policy-Version-Felder zu Beziehungsschreibvorgängen hinzugefügt.', 'Seed- und Konsolidierungsskripte aktualisiert, damit sie die neuen Indizes konsistent füllen.', 'Reconciliation-Befehl mit Bericht, Reparatur, JSON, Bucket-Zählungen und CI-freundlichen Exit-Codes gebaut.', 'Nur-Bericht ausgeführt, bekannte Abweichung repariert, Bericht erneut ausgeführt und sauberen Endzustand verifiziert.', 'Gezielte PHPUnit-Beziehungs-/Cache-/Site-Suite nach der Reparatur ausgeführt.'],
    ...
    'why_p' => ['Payroll-Zusammenarbeit ist sicherheitssensibel. Dashboards, Einladungen, Zugriffsgenehmigungen, Verschlüsselungszustimmung und Widerruf hängen alle von sauberen Beziehungssemantiken ab.', 'Das neue Modell gibt PayCal eine stärkere operative Grenze:', 'Diese Trennung hält Dashboard-Mitgliederzahlen korrekt, verhindert, dass ausstehender Workflow-Status als aktiver Zugriff behandelt wird, hält terminale Zustände aus Live-Indizes heraus und gibt Betreibern eine sichere Möglichkeit, Redis-Abweichungen ohne Änderung von Autoritätsdatensätzen zu reparieren.'],
    'not_l' => ['Es war kein Hinweis auf Korruption der Eigentümerrolle. Das Audit fand null Eigentümerverletzungen.', 'Es war kein Hinweis darauf, dass widerrufene oder abgelehnte Nutzer in Live-Mitgliederindizes auftauchten. Terminal-Leakage-Buckets waren null.', 'Es war keine Umschreibung kanonischer Beziehungsdatensätze. Die Reparatur glich nur abgeleitete Redis-Sets ab.', 'Es war keine Änderung der öffentlichen Geschäftssuchrichtlinie. Öffentliche Suche bleibt durch Sichtbarkeit, Abonnementstatus, Freigabestatus und Listing-Status geregelt.'],
    'future_l' => ['Vor großen Reparaturen einen Redis-Snapshot oder ein Backup erstellen.', 'Nur-Bericht ausführen und das JSON-Artefakt speichern.', '<code>--fix</code> zuerst in einer Nicht-Produktionsumgebung ausführen.', 'Nur-Bericht erneut ausführen und <code>drift=0</code>, <code>owner_violations=0</code> und <code>other=0</code> bestätigen.', 'Gezielte Beziehungs- und Business-Workflow-Tests ausführen.', 'Dashboard-Zählungen, ausstehende Anfragen, Einladungsannahme, Zugriffsgenehmigung, Widerruf und öffentliche Suche per Smoke-Test prüfen.', 'Produktionsreparatur nur in einem ruhigen Fenster ausführen, nachdem der Nicht-Produktionslauf unauffällig war.'],
  ],
];

$copy['es'] = [
  'title' => 'Cómo PayCal mantiene las membresías de negocio precisas y auditables',
  'current' => 'Integridad de membresías de negocio',
  'meta' => 'Cómo PayCal separa los registros de ciclo de vida de relaciones de negocio de los índices de búsqueda en Redis, audita la deriva de índices derivados y mantiene limpias la membresía activa, el acceso pendiente y la autoridad de propietarios.',
  'deck' => 'La membresía de negocio es más que una lista de personas. En junio de 2026 reforzamos cómo PayCal Business representa membresías, solicitudes de acceso, invitaciones y acceso activo en Redis. Este artículo explica el modelo, la deriva que encontramos, el comando de reparación y por qué el cambio importa para privacidad, exactitud de paneles y auditoría futura.',
] + $copy['de'];
$copy['fr'] = [
  'title' => 'Comment PayCal garde les adhésions Business exactes et auditables',
  'current' => 'Intégrité des adhésions Business',
  'meta' => 'Comment PayCal sépare les enregistrements de cycle de vie des relations Business des index Redis, audite la dérive des index dérivés et garde propres l’adhésion active, l’accès en attente et l’autorité propriétaire.',
  'deck' => 'Une adhésion Business est plus qu’une liste de personnes. En juin 2026, nous avons renforcé la façon dont PayCal Business représente les adhésions, demandes d’accès, invitations et accès actifs dans Redis. Cet article explique le modèle, la dérive trouvée, la commande de réparation et l’importance de ce changement pour la confidentialité, les tableaux de bord et l’auditabilité.',
] + $copy['de'];
$copy['it'] = [
  'title' => 'Come PayCal mantiene accurate e verificabili le appartenenze Business',
  'current' => 'Integrità delle appartenenze Business',
  'meta' => 'Come PayCal separa i record del ciclo di vita delle relazioni Business dagli indici Redis, verifica la deriva degli indici derivati e mantiene puliti appartenenza attiva, accesso in sospeso e autorità del proprietario.',
  'deck' => 'L’appartenenza a un Business è più di un elenco di persone. Nel giugno 2026 abbiamo rafforzato il modo in cui PayCal Business rappresenta appartenenze, richieste di accesso, inviti e accesso attivo in Redis. Questo articolo spiega il modello, la deriva trovata, il comando di riparazione e perché il cambiamento conta per privacy, dashboard e audit futuri.',
] + $copy['de'];
$copy['nl'] = [
  'title' => 'Hoe PayCal bedrijfsleden nauwkeurig en controleerbaar houdt',
  'current' => 'Integriteit van bedrijfsleden',
  'meta' => 'Hoe PayCal levenscyclusrecords van bedrijfsrelaties scheidt van Redis-lookupindexen, afgeleide indexdrift controleert en actief lidmaatschap, wachtende toegang en eigenaarautoriteit schoon houdt.',
  'deck' => 'Bedrijfslidmaatschap is meer dan een lijst mensen. In juni 2026 hebben we aangescherpt hoe PayCal Business lidmaatschappen, toegangsaanvragen, uitnodigingen en actieve toegang in Redis voorstelt. Dit artikel legt het model uit, de drift die we vonden, de reparatieopdracht en waarom dit telt voor privacy, dashboards en auditbaarheid.',
] + $copy['de'];
$copy['pt'] = [
  'title' => 'Como o PayCal mantém associações de negócios precisas e auditáveis',
  'current' => 'Integridade de associações de negócios',
  'meta' => 'Como o PayCal separa registros canônicos de ciclo de vida de relacionamentos de negócios dos índices Redis, audita deriva de índices derivados e mantém limpos associação ativa, acesso pendente e autoridade de proprietário.',
  'deck' => 'Associação de negócio é mais do que uma lista de pessoas. Em junho de 2026 reforçamos como o PayCal Business representa associações, solicitações de acesso, convites e acesso ativo no Redis. Este artigo explica o modelo, a deriva encontrada, o comando de reparo e por que isso importa para privacidade, dashboards e auditoria futura.',
] + $copy['de'];
$copy['tl'] = [
  'title' => 'Paano pinananatiling tama at naa-audit ng PayCal ang Business membership',
  'current' => 'Integridad ng Business membership',
  'meta' => 'Kung paano hinihiwalay ng PayCal ang lifecycle records ng business relationship mula sa Redis lookup indexes, ina-audit ang derived-index drift, at pinananatiling malinis ang active membership, pending access, at owner authority.',
  'deck' => 'Ang business membership ay higit pa sa listahan ng mga tao. Noong Hunyo 2026, pinatibay namin kung paano kinakatawan ng PayCal Business ang membership, access requests, invites, at active access sa Redis. Ipinapaliwanag ng artikulong ito ang modelo, ang drift na nakita namin, ang repair command, at kung bakit mahalaga ito sa privacy, dashboard correctness, at future auditability.',
] + $copy['de'];
$copy['tr'] = [
  'title' => 'PayCal işletme üyeliğini nasıl doğru ve denetlenebilir tutar',
  'current' => 'İşletme üyeliği bütünlüğü',
  'meta' => 'PayCal’ın işletme ilişki yaşam döngüsü kayıtlarını Redis arama indekslerinden nasıl ayırdığı, türetilmiş indeks sapmasını nasıl denetlediği ve aktif üyelik, bekleyen erişim ile sahip yetkisini nasıl temiz tuttuğu.',
  'deck' => 'İşletme üyeliği bir kişi listesinden fazlasıdır. Haziran 2026’da PayCal Business’ın üyelikleri, erişim isteklerini, davetleri ve aktif erişimi Redis’te nasıl temsil ettiğini sıkılaştırdık. Bu makale modeli, bulduğumuz sapmayı, onarım komutunu ve değişikliğin gizlilik, pano doğruluğu ve gelecekteki denetlenebilirlik için neden önemli olduğunu açıklar.',
] + $copy['de'];
$copy['hi'] = [
  'title' => 'PayCal Business सदस्यता को सटीक और ऑडिट योग्य कैसे रखता है',
  'current' => 'Business सदस्यता अखंडता',
  'meta' => 'PayCal कैसे business relationship lifecycle records को Redis lookup indexes से अलग करता है, derived-index drift का audit करता है, और active membership, pending access तथा owner authority को साफ रखता है।',
  'deck' => 'Business membership केवल लोगों की सूची नहीं है। जून 2026 में हमने मजबूत किया कि PayCal Business Redis में membership, access requests, invites और active access को कैसे दर्शाता है। यह लेख मॉडल, मिली drift, repair command और privacy, dashboard correctness तथा future auditability के लिए इसके महत्व को समझाता है।',
] + $copy['de'];
...
$localizedOverrides = [
  'es' => [
    ...
    'summary_rows' => [
      ['Área afectada', 'Registros de ciclo de vida de relaciones de PayCal Business e índices de búsqueda en Redis'],
      ...
      ['Corrección principal', 'Los hashes de relación siguen siendo canónicos; miembros activos, relaciones pendientes e índices no terminales están separados'],
      ['Herramienta operativa', '<code>scripts/paycal business:relationships:audit</code> audita y repara deriva de índices derivados'],
    ...
    'problem_p' => [
      'PayCal Business usa Redis para búsquedas rápidas de relaciones. El registro canónico es un hash como <code>business:relationship:{businessId}:{userUUID}</code>. Ese hash guarda el ciclo de vida de una relación: rol, estado, alcances, marcas de tiempo, metadatos de invitación o solicitud y campos de auditoría.',
      'El problema no era el hash canónico. El problema era la deriva semántica en conjuntos derivados de Redis usados como índices de búsqueda. Históricamente, <code>business:members:{businessId}</code> y <code>business:user:{userUUID}</code> a veces se usaban como índices amplios de relación. Eso hacía demasiado fácil que código antiguo tratara una relación pendiente u obsoleta como membresía activa.',
      'La distinción importa. Una solicitud de acceso pendiente no es membresía activa. Una relación aceptada no es automáticamente acceso a datos cifrados si el consentimiento y el estado de key-wrap requeridos no son válidos. Un conjunto derivado de Redis no es autoridad; es un índice reemplazable.',
    ],
    'model_intro' => 'El modelo endurecido separa el estado del ciclo de vida del estado de acceso activo:',
    'model_after' => 'Así cada conjunto Redis tiene un solo significado. Los conteos del panel pueden usar índices de miembros activos. Las colas pendientes pueden usar <code>business:pending:{businessId}</code>. La interfaz puede usar índices de relación sin conceder acceso activo.',
    'drift_intro' => 'Una entrada de índice obsoleta se ve así:',
    'drift_second' => 'Otro ejemplo reparable es una relación activa a la que le faltan índices activos:',
    'drift_after' => 'En ambos casos, la reparación no inventa una relación. Solo reconcilia conjuntos derivados contra el hash canónico de relación.',
    ...
    'repair_p' => ['Creamos <code>scripts/business-relationships-audit.php</code> y lo expusimos mediante el despachador interno existente:', 'El comando tiene dos modos:', 'El comando nunca selecciona propietarios automáticamente, nunca reescribe autoridad de roles y nunca crea relaciones canónicas. Solo reconcilia índices desechables contra hashes canónicos.'],
    'modes' => ['<strong>Solo informe:</strong> escanea hashes de relación y reporta deriva sin mutar Redis.', '<strong>Reparación:</strong> corrige deriva conocida agregando o quitando miembros de conjuntos Redis.'],
    ...
    'actions_l' => ['Separamos índices de membresía activa de índices de ciclo de vida de relación.', 'Agregamos <code>business:relationships:{businessId}</code> y <code>business:relationships:user:{userUUID}</code> para búsqueda no terminal.', 'Agregamos <code>business:pending:{businessId}</code> para visibilidad de flujos pendientes.', 'Actualizamos el escritor central de relaciones para mantener todos los conjuntos derivados desde una sola ruta.', 'Endurecimos transiciones terminales para que relaciones revocadas, rechazadas, vencidas o retiradas no pasen directo a activas.', 'Agregamos campos de preset de alcance y versión de política.', 'Actualizamos scripts de semillas y consolidación.', 'Construimos el comando con informe, reparación, JSON, buckets y códigos de salida para CI.', 'Ejecutamos informe, reparación, nuevo informe y verificamos estado limpio.', 'Ejecutamos la suite PHPUnit enfocada en relaciones/cache/sites.'],
...
$localizedOverrides['it']['summary_rows'] = [['Area interessata', 'Record del ciclo di vita delle relazioni PayCal Business e indici Redis'], ['Problema', 'Vecchi set Redis derivati potevano mescolare appartenenza attiva, stato in sospeso e voci storiche obsolete'], ['Correzione principale', 'Gli hash di relazione restano canonici; membri attivi, relazioni in sospeso e indici non terminali sono separati'], ['Strumento operativo', '<code>scripts/paycal business:relationships:audit</code> verifica e ripara la deriva degli indici'], ['Prima della riparazione', '18.349 derive note riparabili, zero violazioni owner, zero deriva sconosciuta'], ['Dopo la riparazione', '0 deriva, 0 violazioni owner, 0 deriva sconosciuta']];
$localizedOverrides['it']['problem_p'] = ['PayCal Business usa Redis per lookup rapidi. Il record canonico è un hash come <code>business:relationship:{businessId}:{userUUID}</code>, che conserva ruolo, stato, scope, timestamp, metadati di invito o richiesta e campi di audit.', 'Il problema non era l’hash canonico, ma la deriva semantica nei set Redis derivati. In passato <code>business:members:{businessId}</code> e <code>business:user:{userUUID}</code> potevano essere usati come indici ampi, facendo sembrare attiva una relazione in sospeso o obsoleta.', 'La distinzione conta. Una richiesta in sospeso non è appartenenza attiva. Una relazione accettata non dà automaticamente accesso ai dati cifrati senza consenso e stato key-wrap validi. Un set Redis derivato non è autorità; è un indice sostituibile.'];
$localizedOverrides['it']['model_intro'] = 'Il modello rafforzato separa ciclo di vita e accesso attivo:';
$localizedOverrides['it']['model_after'] = 'Ogni set Redis ha così un solo significato. I conteggi dashboard usano gli indici attivi, le code pendenti usano <code>business:pending:{businessId}</code> e la UI può leggere relazioni senza concedere accesso attivo.';
$localizedOverrides['it']['drift_intro'] = 'Una voce di indice obsoleta appare così:';
$localizedOverrides['it']['drift_second'] = 'Un altro esempio riparabile è una relazione attiva priva degli indici attivi:';
$localizedOverrides['it']['drift_after'] = 'In entrambi i casi la riparazione non crea una relazione; riconcilia solo set derivati con l’hash canonico.';
...
$localizedOverrides['it']['repair_p'] = ['Abbiamo creato <code>scripts/business-relationships-audit.php</code> e lo abbiamo esposto tramite il dispatcher interno:', 'Il comando ha due modalità:', 'Il comando non sceglie mai owner automaticamente, non riscrive autorità di ruolo e non crea relazioni canoniche. Riconcilia solo indici usa e getta con hash canonici.'];
...
$localizedOverrides['it']['actions_l'] = ['Separati gli indici di appartenenza attiva dagli indici di ciclo di vita.', 'Aggiunti gli indici non terminali <code>business:relationships:{businessId}</code> e <code>business:relationships:user:{userUUID}</code>.', 'Aggiunto <code>business:pending:{businessId}</code> per i workflow in sospeso.', 'Centralizzata la manutenzione dei set derivati.', 'Rafforzate le transizioni terminali.', 'Aggiunti campi di versione scope e policy.', 'Aggiornati seed e consolidamento.', 'Creato comando con report, fix, JSON, bucket e codici CI.', 'Eseguiti report, fix, nuovo report e verifica finale.', 'Eseguita la suite PHPUnit mirata.'];
...
$localizedOverrides['it']['not_l'] = ['Non era corruzione del ruolo owner: zero violazioni.', 'Non era leakage di utenti revocati o rifiutati: bucket terminali a zero.', 'Non era riscrittura delle relazioni canoniche: solo indici derivati riconciliati.', 'Non era modifica della ricerca pubblica, che resta governata dalle policy esistenti.'];
...
$localizedOverrides['nl']['summary_rows'] = [['Betrokken gebied', 'PayCal Business-relaties en Redis-lookupindexen'], ['Probleem', 'Oude afgeleide Redis-sets konden actief lidmaatschap, wachtstatus en verouderde items mengen'], ['Kernoplossing', 'Relatiehashes blijven canoniek; actieve leden, wachtende relaties en niet-terminale indexen zijn gescheiden'], ['Operationele tool', '<code>scripts/paycal business:relationships:audit</code> controleert en herstelt indexdrift'], ['Voor herstel', '18.349 bekende herstelbare driftitems, nul eigenaarsschendingen, nul onbekende drift'], ['Na herstel', '0 drift, 0 eigenaarsschendingen, 0 onbekende drift']];
$localizedOverrides['nl']['problem_p'] = ['PayCal Business gebruikt Redis voor snelle relatie-lookups. Het canonieke record is een hash zoals <code>business:relationship:{businessId}:{userUUID}</code> met rol, status, scopes, tijdstempels, uitnodigings- of aanvraagmetadata en auditvelden.', 'Het probleem was niet de canonieke hash, maar semantische drift in afgeleide Redis-sets. Vroeger konden <code>business:members:{businessId}</code> en <code>business:user:{userUUID}</code> als brede relatie-indexen worden gebruikt, waardoor oude code een wachtende of oude relatie als actief kon behandelen.', 'Dat onderscheid is belangrijk. Een wachtende aanvraag is geen actief lidmaatschap. Een geaccepteerde relatie geeft niet automatisch versleutelde gegevenstoegang zonder geldige toestemming en key-wrapstatus. Een afgeleide Redis-set is geen autoriteit; het is een vervangbare index.'];
$localizedOverrides['nl']['model_intro'] = 'Het geharde model scheidt levenscyclusstatus van actieve toegang:';
$localizedOverrides['nl']['model_after'] = 'Elke Redis-set heeft nu één betekenis. Dashboardtellingen gebruiken actieve indexen, wachtrijen gebruiken <code>business:pending:{businessId}</code>, en de UI kan relaties lezen zonder actieve toegang te geven.';
$localizedOverrides['nl']['drift_intro'] = 'Een verouderde indexentry ziet er zo uit:';
$localizedOverrides['nl']['drift_second'] = 'Een tweede herstelbaar voorbeeld is een actieve relatie zonder actieve indexen:';
$localizedOverrides['nl']['drift_after'] = 'In beide gevallen maakt herstel geen relatie aan; het stemt afgeleide sets af op de canonieke hash.';
...
$localizedOverrides['nl']['repair_p'] = ['We maakten <code>scripts/business-relationships-audit.php</code> en ontsloten dit via de interne command dispatcher:', 'De opdracht heeft twee modi:', 'De opdracht kiest nooit automatisch eigenaars, herschrijft geen rolautoriteit en maakt geen canonieke relaties. Ze stemt alleen wegwerpindexen af op canonieke hashes.'];
...
$localizedOverrides['nl']['actions_l'] = ['Actieve lidindexen gescheiden van levenscyclusindexen.', 'Niet-terminale indexen toegevoegd.', '<code>business:pending:{businessId}</code> toegevoegd.', 'Centraal relatiepad bijgewerkt.', 'Terminale overgangen gehard.', 'Scope- en policyversies toegevoegd.', 'Seed- en consolidatiescripts bijgewerkt.', 'Reconciliation-command gebouwd.', 'Rapport, herstel en eindcontrole uitgevoerd.', 'Gerichte PHPUnit-suite uitgevoerd.'];
...
$localizedOverrides['nl']['why_p'] = ['Payroll-samenwerking is securitygevoelig. Dashboards, uitnodigingen, goedkeuringen, encryptietoestemming en intrekking hangen af van schone relatiesemantiek.', 'Het nieuwe model geeft PayCal een sterkere operationele grens:', 'Die scheiding houdt tellingen correct, voorkomt dat pending als actief geldt, houdt terminale statussen uit live indexen en maakt veilig Redis-herstel mogelijk.'];
$localizedOverrides['nl']['not_l'] = ['Geen eigenaarrolcorruptie: nul schendingen.', 'Geen lek van ingetrokken of afgewezen gebruikers: terminale buckets waren nul.', 'Geen herschrijving van canonieke relaties: alleen afgeleide indexen.', 'Geen wijziging aan openbaar zoekbeleid.'];
...
$localizedOverrides['pt']['repair_p'] = ['Criamos <code>scripts/business-relationships-audit.php</code> e expusemos pelo dispatcher interno:', 'O comando tem dois modos:', 'O comando nunca escolhe proprietários automaticamente, não reescreve autoridade de papéis e não cria relações canônicas. Ele só reconcilia índices descartáveis com hashes canônicos.'];
...
$localizedOverrides['tl']['summary_rows'] = [['Apektadong bahagi', 'PayCal Business relationship lifecycle records at Redis lookup indexes'], ['Problema', 'Lumang derived Redis sets ay puwedeng maghalo ng active membership, pending workflow state, at stale historical entries'], ['Core fix', 'Canonical pa rin ang relationship hashes; hiwalay ang active members, pending relationships, at non-terminal lookup indexes'], ['Ops tool', '<code>scripts/paycal business:relationships:audit</code> ang nag-audit at nag-aayos ng drift'], ['Bago ayusin', '18,349 known repairable drift, zero owner violations, zero unknown drift'], ['Pagkatapos ayusin', '0 drift, 0 owner violations, 0 unknown drift']];
$localizedOverrides['tl']['problem_p'] = ['Gumagamit ang PayCal Business ng Redis para sa mabilis na relationship lookup. Ang canonical record ay hash tulad ng <code>business:relationship:{businessId}:{userUUID}</code> na may role, status, scopes, timestamps, metadata, at audit fields.', 'Hindi ang canonical hash ang problema. Ang problema ay semantic drift sa derived Redis sets. Dati, puwedeng gamitin ang <code>business:members:{businessId}</code> at <code>business:user:{userUUID}</code> bilang malawak na relationship indexes, kaya madaling magmukhang active ang pending o stale relationship.', 'Mahalaga ang pagkakaiba. Ang pending access request ay hindi active membership. Ang accepted relationship ay hindi awtomatikong encrypted data access kung walang valid consent at key-wrap state. Ang derived Redis set ay hindi authority; index lang ito.'];
$localizedOverrides['tl']['model_intro'] = 'Pinaghihiwalay ng hardened model ang lifecycle state at active access state:';
$localizedOverrides['tl']['model_after'] = 'May isang kahulugan ang bawat Redis set. Active indexes ang para sa dashboard counts, <code>business:pending:{businessId}</code> ang para sa pending queues, at relationship indexes ang para sa UI na hindi nagbibigay ng active access.';
$localizedOverrides['tl']['drift_intro'] = 'Ganito ang stale index entry:';
$localizedOverrides['tl']['drift_second'] = 'Isa pang repairable example ay active relationship na kulang sa active indexes:';
$localizedOverrides['tl']['drift_after'] = 'Sa dalawang kaso, hindi gumagawa ng relationship ang repair. Ina-align lang nito ang derived sets sa canonical hash.';
...
$localizedOverrides['tl']['repair_p'] = ['Ginawa namin ang <code>scripts/business-relationships-audit.php</code> at inilabas ito sa internal dispatcher:', 'May dalawang mode ang command:', 'Hindi ito auto-pumipili ng owner, hindi nagre-rewrite ng role authority, at hindi gumagawa ng canonical relationships. Inaayos lang nito ang disposable indexes laban sa canonical hashes.'];
...
$localizedOverrides['tl']['why_p'] = ['Security-sensitive ang payroll collaboration. Dashboard, invites, approvals, encryption consent, at revoke ay nakadepende sa malinis na relationship semantics.', 'Mas malinaw na operational boundary ang bagong model:', 'Pinapanatili nitong tama ang counts, hindi ginagawang active ang pending, inaalis ang terminal states sa live indexes, at ligtas na nag-aayos ng Redis drift.'];
$localizedOverrides['tl']['not_l'] = ['Hindi ito owner-role corruption: zero violations.', 'Hindi ito leakage ng revoked o rejected users: zero terminal buckets.', 'Hindi ito rewrite ng canonical relationships: derived indexes lang.', 'Hindi ito pagbabago sa public search policy.'];
...
$localizedOverrides['tr']['summary_rows'] = [['Etkilenen alan', 'PayCal Business ilişki yaşam döngüsü kayıtları ve Redis indeksleri'], ['Sorun', 'Eski türetilmiş Redis setleri aktif üyelik, bekleyen durum ve eski girdileri karıştırabiliyordu'], ['Temel düzeltme', 'İlişki hashleri kanonik kaldı; aktif üyeler, bekleyen ilişkiler ve terminal olmayan indeksler ayrıldı'], ['Operasyon aracı', '<code>scripts/paycal business:relationships:audit</code> sapmayı denetler ve onarır'], ['Onarım öncesi', '18.349 bilinen onarılabilir sapma, sıfır sahip ihlali, sıfır bilinmeyen sapma'], ['Onarım sonrası', '0 sapma, 0 sahip ihlali, 0 bilinmeyen sapma']];
$localizedOverrides['tr']['problem_p'] = ['PayCal Business hızlı aramalar için Redis kullanır. Kanonik kayıt <code>business:relationship:{businessId}:{userUUID}</code> gibi bir hashtir ve rol, durum, kapsamlar, zaman damgaları, metadata ve audit alanlarını saklar.', 'Sorun kanonik hash değildi; türetilmiş Redis setlerinde semantik sapmaydı. Geçmişte <code>business:members:{businessId}</code> ve <code>business:user:{userUUID}</code> geniş ilişki indeksleri gibi kullanılabiliyordu.', 'Bekleyen erişim isteği aktif üyelik değildir. Kabul edilmiş ilişki, geçerli onay ve key-wrap yoksa otomatik şifreli veri erişimi değildir. Türetilmiş Redis seti otorite değil, değiştirilebilir indekstir.'];
$localizedOverrides['tr']['model_intro'] = 'Sertleştirilmiş model yaşam döngüsü ile aktif erişimi ayırır:';
$localizedOverrides['tr']['model_after'] = 'Her Redis setinin tek anlamı vardır. Dashboard sayımları aktif indeksleri, bekleyen kuyruklar <code>business:pending:{businessId}</code> kullanır; UI aktif erişim vermeden ilişki indekslerini okuyabilir.';
$localizedOverrides['tr']['drift_intro'] = 'Eski bir indeks girdisi şöyle görünür:';
$localizedOverrides['tr']['drift_second'] = 'Bir diğer onarılabilir örnek, aktif indeksleri eksik olan aktif ilişkidir:';
$localizedOverrides['tr']['drift_after'] = 'Her iki durumda da onarım ilişki icat etmez; türetilmiş setleri kanonik hash ile uzlaştırır.';
...
$localizedOverrides['tr']['repair_p'] = ['<code>scripts/business-relationships-audit.php</code> oluşturuldu ve iç komut dağıtıcısına eklendi:', 'Komutun iki modu vardır:', 'Komut otomatik sahip seçmez, rol otoritesini yeniden yazmaz ve kanonik ilişki oluşturmaz. Yalnızca atılabilir indeksleri kanonik hashlerle uzlaştırır.'];
...
$localizedOverrides['tr']['actions_l'] = ['Aktif üyelik indeksleri ayrıldı.', 'Terminal olmayan ilişki indeksleri eklendi.', '<code>business:pending:{businessId}</code> eklendi.', 'Merkezi ilişki yazıcı güncellendi.', 'Terminal geçişler sertleştirildi.', 'Kapsam ve politika sürümleri eklendi.', 'Seed ve konsolidasyon scriptleri güncellendi.', 'Reconciliation komutu oluşturuldu.', 'Rapor, onarım ve doğrulama yapıldı.', 'Hedefli PHPUnit paketi çalıştırıldı.'];
...
$localizedOverrides['tr']['why_p'] = ['Payroll işbirliği güvenlik açısından hassastır. Dashboardlar, davetler, onaylar, şifreleme rızası ve iptal temiz ilişki semantiğine bağlıdır.', 'Yeni model PayCal’a daha güçlü bir operasyon sınırı verir:', 'Bu ayrım sayımları doğru tutar, bekleyeni aktif erişim yapmaz, terminal durumları canlı indekslerden çıkarır ve güvenli Redis onarımı sağlar.'];
$localizedOverrides['tr']['not_l'] = ['Sahip rolü bozulması değildi: sıfır ihlal.', 'İptal veya reddedilmiş kullanıcı sızıntısı değildi: terminal bucketlar sıfırdı.', 'Kanonik ilişkiler yeniden yazılmadı: yalnızca türetilmiş indeksler uzlaştırıldı.', 'Genel arama politikası değişmedi.'];
...
$localizedOverrides['hi']['summary_rows'] = [['प्रभावित क्षेत्र', 'PayCal Business relationship lifecycle records और Redis lookup indexes'], ['समस्या', 'पुराने derived Redis sets active membership, pending state और stale entries को मिला सकते थे'], ['मुख्य सुधार', 'Relationship hashes canonical रहे; active members, pending relationships और non-terminal indexes अलग किए गए'], ['Ops tool', '<code>scripts/paycal business:relationships:audit</code> drift को audit और repair करता है'], ['Repair से पहले', '18,349 known repairable drift, zero owner violations, zero unknown drift'], ['Repair के बाद', '0 drift, 0 owner violations, 0 unknown drift']];
$localizedOverrides['hi']['problem_p'] = ['PayCal Business तेज relationship lookup के लिए Redis का उपयोग करता है। Canonical record <code>business:relationship:{businessId}:{userUUID}</code> जैसा hash है, जिसमें role, status, scopes, timestamps, metadata और audit fields रहते हैं।', 'समस्या canonical hash नहीं थी। समस्या derived Redis sets में semantic drift थी। पहले <code>business:members:{businessId}</code> और <code>business:user:{userUUID}</code> को broad relationship indexes की तरह उपयोग किया जा सकता था, जिससे pending या stale relationship active membership जैसी दिख सकती थी।', 'यह अंतर महत्वपूर्ण है। Pending access request active membership नहीं है। Accepted relationship valid consent और key-wrap state के बिना encrypted data access नहीं देता। Derived Redis set authority नहीं है; यह replaceable index है।'];
$localizedOverrides['hi']['model_intro'] = 'Hardened model lifecycle state को active access state से अलग करता है:';
$localizedOverrides['hi']['model_after'] = 'अब हर Redis set का एक अर्थ है। Dashboard counts active indexes पर भरोसा करते हैं, pending queues <code>business:pending:{businessId}</code> इस्तेमाल करती हैं, और UI active access दिए बिना relationship indexes पढ़ सकती है।';
$localizedOverrides['hi']['drift_intro'] = 'Stale index entry ऐसा दिखता है:';
$localizedOverrides['hi']['drift_second'] = 'एक और repairable example active relationship है जिसके active indexes गायब हैं:';
$localizedOverrides['hi']['drift_after'] = 'दोनों मामलों में repair नई relationship नहीं बनाता। वह केवल derived sets को canonical hash से reconcile करता है।';
...
$localizedOverrides['hi']['repair_p'] = ['हमने <code>scripts/business-relationships-audit.php</code> बनाया और इसे existing internal command dispatcher से expose किया:', 'Command के दो modes हैं:', 'Command कभी owner auto-select नहीं करता, role authority rewrite नहीं करता, और canonical relationships नहीं बनाता। यह केवल disposable indexes को canonical hashes से reconcile करता है।'];
...
$localizedOverrides['hi']['actions_l'] = ['Active membership indexes को lifecycle indexes से अलग किया।', 'Non-terminal relationship indexes जोड़े।', '<code>business:pending:{businessId}</code> जोड़ा।', 'Central relationship writer update किया।', 'Terminal transitions harden किए।', 'Scope और policy versions जोड़े।', 'Seed और consolidation scripts update किए।', 'Reconciliation command बनाया।', 'Report, fix और final verification चलाया।', 'Targeted PHPUnit suite चलाई।'];
...
$localizedOverrides['hi']['why_p'] = ['Payroll collaboration security-sensitive है। Dashboards, invites, approvals, encryption consent और revoke साफ relationship semantics पर निर्भर हैं।', 'नया model PayCal को मजबूत operational boundary देता है:', 'यह separation counts सही रखता है, pending को active access नहीं मानता, terminal states को live indexes से बाहर रखता है, और Redis drift को authority mutate किए बिना repair करने देता है।'];
$localizedOverrides['hi']['not_l'] = ['यह owner-role corruption नहीं था: zero violations।', 'यह revoked या rejected users का leakage नहीं था: terminal buckets zero थे।', 'यह canonical relationships rewrite नहीं था: केवल derived indexes reconcile हुए।', 'यह public search policy change नहीं था।'];
...
<article class="article doc-article">
  ...
  <div class="doc-article-body">
    <section class="doc-section highlight">
      ...
      <table class="doc-table" aria-label="Redis relationship reconciliation summary"><tbody><?php rr_rows($t['summary_rows']); ?></tbody></table>
    </section>
...
SREM business:user:user-123 ORGabc</code></pre></div>
      ...
      <div class="doc-code-block" data-label="Missing active indexes"><pre><code>HGETALL business:relationship:ORGabc:user-456
...
SADD business:user:user-456 ORGabc</code></pre></div>
    ...
    <section class="doc-section">
      ...
      <table class="doc-table" aria-label="Pre-repair Redis relationship audit buckets">
        ...
        <tbody>
          <tr><td><code>stale_index_without_relationship</code></td><td>14,886</td><td>Old derived indexes pointed to relationships that no longer existed</td></tr>
          <tr><td><code>relationship_lookup_missing</code></td><td>2,908</td><td>New non-terminal relationship lookup indexes needed backfill</td></tr>
          <tr><td><code>active_missing_member_index</code></td><td>46</td><td>Active relationships missing the business-side active member index</td></tr>
          <tr><td><code>active_missing_user_index</code></td><td>493</td><td>Active relationships missing the user-side active business index</td></tr>
          <tr><td><code>terminal_in_relationship_index</code></td><td>0</td><td>No revoked, rejected, expired, or withdrawn relationships leaked into relationship lookup indexes</td></tr>
          <tr><td><code>terminal_in_member_index</code></td><td>0</td><td>No terminal relationships leaked into active member indexes</td></tr>
    ...
    <section class="doc-section">
      ...
      <div class="doc-code-block" data-label="Internal ops command"><pre><code>scripts/paycal business:relationships:audit</code></pre></div>
    ...
    <section class="doc-section">
      ...
      <div class="operator-command-list">
        <?php
        $commands = [
          ['Report-only', 'scripts/paycal business:relationships:audit --json'],
          ['Scoped report', 'scripts/paycal business:relationships:audit --business ORGabc123 --json'],
          ['Before artifact', "scripts/paycal business:relationships:audit --json \\\n  &gt; tmp/business-relationships-report-before.json"],
          ['Controlled repair', "scripts/paycal business:relationships:audit --fix --json \\\n  &gt; tmp/business-relationships-fix.json"],
          ['After artifact', "scripts/paycal business:relationships:audit --json \\\n  &gt; tmp/business-relationships-report-after.json"],
...
other=0</code></pre></div>
    ...
    <section class="doc-section">
      ...
      <div class="doc-code-block" data-label="Design principle"><pre><code>relationship = lifecycle record
