...
namespace PayCal\Infrastructure\Transaction;
 ...
 *
 * Purpose: Redis-backed transaction model for multi-step account recovery,
 * including state progression, proof windows, and bootstrap token issuance.
 *
 * Developer notes:
 * - Recovery transaction state is security-sensitive and must preserve
 *   ordered progression, expiry semantics, and replay resistance.
 * - Controllers and services should rely on this model for authoritative
 *   recovery-state transitions rather than duplicating transition logic.
 *
 * Architectural role:
 * - Infrastructure transaction object coordinating persistence-backed state
 *   across recovery workflows.
 * - Encapsulates recovery state transitions outside the HTTP layer.
 *
 * @category   Infrastructure
 * @package    PayCal\Infrastructure\Transaction
 * @subpackage Core
...
final class AccountRecoveryTransaction
{
  ...
  /**
   * Handles markEmailVerifiedByMagicLink operation.
   */
  public function markEmailVerifiedByMagicLink(string $fingerprintHash, string $ipClass): bool
  {
  ...
  public function elevateForMagicLinkPasskey(string $fingerprintHash, string $ipClass): bool
  {
    ...
    if (!in_array($this->status(), [self::STATUS_EMAIL_VERIFIED, self::STATUS_PROOF_VERIFIED, self::STATUS_BOOTSTRAP_ISSUED], true)) {
      return false;
    }

    if ($this->status() === self::STATUS_EMAIL_VERIFIED) {
      $this->write([
        'status' => self::STATUS_PROOF_VERIFIED,
        'proof_nonce_hash' => '',
        'proof_nonce_expires_at' => '0',
      ]);
    }
  ...
  {
    if (!in_array($this->status(), [self::STATUS_PROOF_VERIFIED, self::STATUS_BOOTSTRAP_ISSUED], true) || $this->isExpired()) {
      return false;
    }
