...
final class BusinessNotificationService
{
  ...
  public function summarizeUnreadForUser(string $userUUID): array
  {
    ...
    $raw = Database::hgetall(Keys::businessNotificationUnreadByUser($userUUID));
    $byOrg = [];
    $total = 0;
    ...
    Database::set(Keys::businessNotificationTotalByUser($userUUID), (string) $total, 86400);
   ...
   * @return array{total_unread: int, by_org: array<string, int>}
   */
  public function markBusinessRead(string $userUUID, string $businessId): array
  {
    return $this->markOrganizationRead($userUUID, $businessId);
  }
  ...
  public function markOrganizationRead(string $userUUID, string $orgId): array
  {
    ...
    Database::hdel(Keys::businessNotificationUnreadByUser($userUUID), $orgId);
    Database::set(Keys::businessNotificationLastRead($orgId, $userUUID), date('c'), 365 * 24 * 3600);
  ...
  {
    $activeMembers = BusinessMemberRepository::forBusiness($orgId, null, 'active');
    $roleIndex = [];
  ...
  private function incrementUnread(string $userUUID, string $orgId): void
  {
    ...
    $key = Keys::businessNotificationUnreadByUser($userUUID);
    ...
    $totalKey = Keys::businessNotificationTotalByUser($userUUID);
