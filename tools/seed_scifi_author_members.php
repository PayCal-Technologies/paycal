...
/**
 * seed_scifi_author_members.php — 150 Sci-Fi Author Fake Users + Business Membership
 *
 * Purpose: Seeds 150 additional fake users (subtle variations of famous sci-fi author
 *          names), then attaches them to a target business workspace. Pairs with
 *          seed_fake_users.php (10) and seed_famous_actor_members.php (90) for 250
 *          total seeded members.
 *
 * Usage:
 *   php tools/seed_scifi_author_members.php
 *   php tools/seed_scifi_author_members.php ORG498fa7662f3ac4b591
 *   php tools/seed_scifi_author_members.php ORG498fa7662f3ac4b591 --dry-run
 *   php tools/seed_scifi_author_members.php ORG498fa7662f3ac4b591 --drop
 *
 * After seeding users, generate 2026 work history for all members:
...
const SCIFI_UUID_START = 0x6b; // fade006b … fade0100 (150 users)

/** @var list<array{first: string, last: string, inspiration: string, country: string}> */
$authorBases = require __DIR__ . '/scifi_author_users_data.php';

if (count($authorBases) !== 150) {
  fwrite(STDERR, 'Error: scifi_author_users_data.php must define exactly 150 users (got ' . count($authorBases) . ").\n");
  exit(1);
}
 ...
 */
function buildScifiAuthorUsers(array $bases, array $locales, array $jobTitles, array $departments, array $businessRoles, array $roleMap): array
{
  $users = [];
  $idx   = SCIFI_UUID_START;
  ...
  foreach ($bases as $i => $base) {
    ...
    mt_srand(crc32("phone-scifi-{$uuid}"));
...
$users = buildScifiAuthorUsers($authorBases, $locales, $jobTitles, $departments, $businessRoles, $roleMap);

if ($drop) {
  echo "── Dropping sci-fi author fake users ───────────────────────────\n";
  foreach ($users as $u) {
...
if ($dryRun) {
  echo "── Dry-run: would seed 150 sci-fi author users ────────────────────\n";
  foreach ($users as $u) {
    printf(
      "  %-28s  %-12s  %-3s  %-32s  %s\n",
      $u['full_name'],
...
echo "── Seeding 150 sci-fi author fake users ───────────────────────────\n";
