...
- Fixes the biweekly pay-period DST bug that could affect earnings grouping and lazy report rendering, with an America/Edmonton spring DST regression and the affected lazy-render test re-enabled.
- Documents removed old aliases, paths, redirects, method wrappers, placeholder classes, Redis compatibility branches, generated TODO noise, and the policies used to decide what stayed guarded.
- Records the protected-data, pay-period, Redis/connection, crypto/plaintext, security, and settings regression suites now guarding the release.
