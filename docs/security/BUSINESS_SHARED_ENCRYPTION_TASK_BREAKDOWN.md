# Business Shared Encryption Task Breakdown

Status: Execution plan
Date: 2026-04-07
Depends on: `docs/security/BUSINESS_SHARED_ENCRYPTION_IMPLEMENTATION_SPEC.md`
...
Files:
- `html/src/Domain/BusinessDiscoveryService.php`
- `html/src/Domain/Constants/Keys.php`

Tasks:
1. Add membership states: `pending`, `consented`, `active`, `suspended`, `revoked`.
2. Add business encryption mode constants: `personal`, `business`.
3. Add business DEK segment constants: `current_period`, `archive`.
4. Add Redis key builders in `Keys.php` for:
- consent records and indexes
- business DEK registry/version
- business DEK wraps
...
Tasks:
1. Add strict metadata validator for business-mode envelope fields:
- `encryption_mode`
- `business_id`
- `segment`
- `key_version`
- `dek_id`
- `needs_rewrap`
2. Keep backward compatibility behind a feature gate (reject malformed business-mode payloads when enabled).
...
Tasks:
1. Add methods:
- `recordBusinessConsent(...)`
- `loadActiveBusinessConsent(...)`
...
Files:
- `html/src/Domain/BusinessDiscoveryService.php`
- `html/src/Controllers/BusinessDiscoveryController.php`
...
## Phase C - Business DEK Lifecycle Service

### C1. Business DEK version and wrap management

Files:
- `html/src/Domain/BusinessDiscoveryService.php`
- New file: `html/src/Domain/BusinessEncryptionService.php`
...
- `html/src/Controllers/BusinessDiscoveryController.php`
- `html/src/Domain/BusinessDiscoveryService.php`
- `html/src/Domain/BusinessEncryptionService.php`

Tasks:
1. Add endpoint:
- `POST businesses/{businessID}/keys/rotate`
2. Validate caller role/scope (`business.settings.write` or higher policy).

Acceptance:
- Rotation updates current version atomically per segment.

### D3. Business-mode work read/write endpoints

Files:
- New file: `html/src/Controllers/BusinessWorkController.php` (preferred)
- OR extend: `html/src/Controllers/CalendarController.php` with explicit business mode branch

Tasks:
1. Add read endpoint:
- `POST business/work/read`
2. Add write endpoint:
- `POST business/work/write`
3. Server-side validations:
- active membership state
- scope checks (`work.read`, `work.write`)
- target member belongs to business
...
2. Add explicit business-mode save path to pass owner/target context and metadata checks.
...
Files:
- `html/businesses/index.php`
- `html/js/businesses/index.php`
...
Tasks:
1. Show `Shared with Business` indicator in business mode.
2. Show business context (business name, segment, key-version migration notices).

Acceptance:
- Users can distinguish personal vs business encryption mode at save/read time.
...
Acceptance:
- All event types visible through existing business audit timeline/grid mechanisms.
...
Files (new):
- `html/tests/Unit/BusinessEncryptionServiceTest.php`
- `html/tests/Unit/BusinessConsentBindingTest.php`
- `html/tests/Unit/WorkEntryBusinessEnvelopeValidationTest.php`
...
Files (new):
- `html/tests/Integration/BusinessMembershipConsentIntegrationTest.php`
- `html/tests/Integration/BusinessKeyRotationIntegrationTest.php`
- `html/tests/Integration/BusinessWorkReadWriteIntegrationTest.php`

Coverage:
- join without consent denied
- revoke triggers rotation and blocks future decrypt
- business read/write scope gating
...
1. Confirm final route naming convention (`businesses/{businessId}/...` vs top-level `/business/...`).
